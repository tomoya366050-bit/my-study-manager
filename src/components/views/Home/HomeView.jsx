...
const HomeView = ({ logs, categories, dailyGoalMin, onSaveGoal }) => {
  ...
  // --- 今日の学習統計 ---
  const todaySec = logs
    .filter(l => DateUtils.getDateKey(safeGetDate(l.createdAt)) === todayKey)
  ...
  const weeklyData = weekRange.map(date => {
    ...
    const daySec = currentWeekLogs
      .filter(l => DateUtils.getDateKey(safeGetDate(l.createdAt)) === dKey)
      .reduce((s, l) => s + l.duration, 0);
    return { 
      day: DateUtils.weekLabels[date.getDay()], 
      dateLabel: `${date.getMonth() + 1}/${date.getDate()}`, 
      minutes: Math.floor(daySec / 60) 
    };
  });
  ...
  const monthSec = logs
    .filter(l => safeGetDate(l.createdAt).getMonth() === now.getMonth())
    .reduce((s, l) => s + l.duration, 0);
    ...
    <div key="home">
      ...
      {isGoalSettingOpen && (
        <ChicCard>
          ...
          <ChicButton onClick={() => { 
            // ★追加：ValidationUtils での数値チェック
            if (!ValidationUtils.isValidNumber(tempGoal)) {
